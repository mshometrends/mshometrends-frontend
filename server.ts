...
async function startServer() {
  ...
  // Dynamically reflects requesting origin (e.g. https://mshometrends.vercel.app, localhost, previews)
  app.use((req, res, next) => {
    ...
    if (requestHeaders) {
      ...
      res.setHeader(
        ...
        'Content-Type, Authorization, x-admin-key, x-admin-token, X-Requested-With, Accept, Origin, X-CSRF-Token, Accept-Version, Content-Length, Content-MD5, Date, X-Api-Version'
      );
    }
    res.setHeader('Access-Control-Expose-Headers', 'Content-Type, Authorization, Content-Disposition');
    res.setHeader('Access-Control-Max-Age', '86400');

    // Fast preflight resolution
