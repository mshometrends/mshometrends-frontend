...
export const ProductCard: React.FC<ProductCardProps> = ({ product, badgeText }) => {
  ...
  const mainImage = (Array.isArray(product.images) && product.images.length > 0 && product.images[0])
    ? product.images[0]
    : (product as any).image || (product as any).imageUrl || 'https://images.unsplash.com/photo-1578749556568-bc2c40e68b61?auto=format&fit=crop&w=600&q=80';
  ...
  return (
    <motion.div
      ...
      className="group relative flex flex-col justify-between rounded-2xl sm:rounded-3xl bg-[#F4F1EA]/80 hover:bg-[#EFEBE1] border border-stone-200/80 hover:border-[#D4AF37]/50 p-3.5 sm:p-5 transition-all duration-400 hover:shadow-xl hover:shadow-[#0A3825]/5 cursor-pointer overflow-hidden h-full w-full select-none"
    ...
    >
      ...
      <div className="relative w-full aspect-square my-2 flex items-center justify-center overflow-hidden rounded-2xl bg-white/40">
        <motion.img
          src={mainImage}
          alt={product.name}
