...
import { api, apiClient, ENDPOINTS } from '../api';
import {
  Search,
  Filter,
  Eye,
...
export const AdminOrdersView: React.FC = () => {
  ...
  const handleConfirmPayment = async (orderId: string) => {
    try {
      ...
      const res = await api.orders.confirmPayment(orderId, { verifiedBy: 'Admin', adminNote });
  ...
  const handleRejectPayment = async (orderId: string) => {
    ...
    try {
      ...
      const res = await api.orders.rejectPayment(orderId, { rejectionReason, adminNote });
  ...
  const handleUpdateStatus = async (orderId: string, newStatus: string) => {
    try {
      ...
      const res = await api.orders.updateStatus(orderId, { orderStatus: newStatus, adminNote });
