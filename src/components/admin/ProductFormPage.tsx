...
export const ProductFormPage: React.FC<ProductFormPageProps> = ({ productToEdit, onClose }) => {
  ...
  const [formData, setFormData] = useState({
    ...
    images: ['https://images.unsplash.com/photo-1615865417236-d67f589c424d?q=80&w=800&auto=format&fit=crop'],
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (validImages.length === 0) {
      showToast('At least one product image is required (max 5 pictures allowed)', 'error');
      return;
    }
      ...
      <form onSubmit={handleSubmit} className="space-y-8 text-xs text-slate-700">
        ...
        <div className="space-y-4 pt-4 border-t border-slate-100">
          <MultiImageUpload
            ...
            maxImages={5}
            label="Product Photography & Media Gallery"
          />
        </div>
