...
// Utility to convert text into a clean URL slug
...
const slugifyWhileTyping = (text: string): string => {
  return text
    .toLowerCase()
    .replace(/[^\w\s-]/g, '')
    .replace(/\s+/g, '-')
...
export const CategoryFormPage: React.FC<CategoryFormPageProps> = ({ categoryToEdit, onClose }) => {
  ...
  useEffect(() => {
    if (categoryToEdit) {
      setFormData({
        ...
        slug: categoryToEdit.slug || slugify(categoryToEdit.name || ''),
        description: categoryToEdit.description || '',
        imageUrl: categoryToEdit.image || '',
      });
      setIsSlugCustomized(Boolean(categoryToEdit.slug));
    } else {
  ...
  const handleNameChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const autoSlug = slugifyWhileTyping(val);
    setFormData((prev) => ({
      ...prev,
      name: val,
      slug: !isSlugCustomized ? autoSlug : prev.slug,
    }));
  };
  ...
  const handleSlugChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!val.trim()) {
      ...
      setFormData((prev) => ({
        ...prev,
        slug: slugifyWhileTyping(prev.name),
      }));
    } else {
      setIsSlugCustomized(true);
      setFormData((prev) => ({
        ...prev,
        slug: slugifyWhileTyping(val),
      }));
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const finalSlug = slugify(formData.slug.trim() || formData.name.trim());
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <input
              type="text"
              required
              placeholder="e.g. Water Bottle, Royal Tea Sets, Serving Platters"
              value={formData.name}
              onChange={handleNameChange}
              className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3.5 py-3 text-slate-800 text-sm font-medium focus:outline-none focus:border-[#D4AF37] focus:bg-white transition-all shadow-xs"
            />
          </div>
          ...
          <div>
            <div className="flex items-center justify-between mb-1.5">
              <label className="block text-slate-700 font-semibold">
                URL Slug <span className="text-[11px] font-normal text-slate-500">(Auto-generated)</span>
              </label>
              {isSlugCustomized && (
                <button
                  type="button"
                  onClick={() => {
                    setIsSlugCustomized(false);
                    setFormData((prev) => ({ ...prev, slug: slugifyWhileTyping(prev.name) }));
                  }}
                  className="text-[11px] text-[#D4AF37] hover:text-[#997300] hover:underline font-semibold cursor-pointer"
                >
                  ↺ Reset to Auto-Slug
                </button>
              )}
            ...
            <input
              ...
              className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3.5 py-3 text-slate-800 font-mono text-xs focus:outline-none focus:border-[#D4AF37] focus:bg-white transition-all shadow-xs"
            />
            <p className="text-[11px] text-slate-500 mt-1.5 flex items-center gap-1 font-sans">
              <span className="text-slate-400">Preview:</span>
              <span className="font-mono text-emerald-700 bg-emerald-50 px-1.5 py-0.5 rounded border border-emerald-200 text-[10.5px]">
