...
export const Header: React.FC = () => {
  ...
  return (
    <>
      ...
      <div className="bg-[#0A3825] text-amber-100 text-[10.5px] sm:text-xs py-1.5 sm:py-2 px-3 sm:px-6 lg:px-10 xl:px-14 font-medium border-b border-[#D4AF37]/30 shadow-sm w-full">
        <div className="max-w-[1760px] 2xl:max-w-[1920px] mx-auto flex items-center justify-between gap-2 text-center sm:text-left">
          <div className="flex items-center gap-1.5 sm:gap-2 min-w-0">
            <Sparkles className="w-3 h-3 text-[#D4AF37] shrink-0" />
            <span className="text-amber-300 font-semibold truncate text-[10.5px] sm:text-xs">
              <span className="hidden xs:inline">Handcrafted </span>Fine Bone China & Royal Tableware
            </span>
          </div>
          <div className="flex items-center gap-3 shrink-0 text-[10.5px] sm:text-xs">
            <a
              href="tel:+923143015526"
            ...
            >
              ...
              <span className="tracking-tight">+92 314 3015526</span>
            </a>
          </div>
        </div>
      </div>
      ...
      <header
        className={`sticky top-0 z-40 transition-all duration-300 w-full ${
          isScrolled
            ? 'bg-white/95 backdrop-blur-md shadow-sm border-b border-[#D4AF37]/30 py-2 sm:py-2.5'
            : 'bg-white border-b border-slate-200/80 py-2.5 sm:py-3'
        }`}
      >
        <div className="max-w-[1760px] 2xl:max-w-[1920px] mx-auto px-3 sm:px-6 lg:px-10 xl:px-14 flex items-center justify-between gap-2 sm:gap-4">
          ...
          <div className="flex items-center gap-2 sm:gap-6 lg:gap-8 min-w-0">
            ...
            <button
              ...
              className="lg:hidden text-[#0A3825] hover:text-[#D4AF37] p-1.5 -ml-1 rounded-lg hover:bg-slate-100 transition-colors shrink-0 cursor-pointer"
            ...
            <div
              ...
              className="cursor-pointer group shrink min-w-0"
            >
              <Logo size="sm" />
            </div>

            {/* Desktop Navigation Links */}
            <nav aria-label="Main Navigation" className="hidden lg:flex items-center gap-5 xl:gap-7 text-[13.5px] font-semibold text-slate-700">
              <a
                href="/"
                onClick={(e) => {
                  e.preventDefault();
              ...
              >
                ...
                <AnimatePresence>
                  {megaMenuOpen && (
                    ...
                    >
                      <div className="col-span-3 grid grid-cols-3 gap-3 border-r border-slate-100 pr-4 max-h-[440px] overflow-y-auto">
                        {categories.map((cat, idx) => {
                          ...
                          return (
                            <a
                              ...
                              className={`flex items-center gap-2.5 p-2 rounded-xl hover:bg-emerald-50/70 cursor-pointer transition-all group border ${
                                isWaterBottle
                                  ? 'bg-emerald-50/50 border-emerald-300/60 hover:border-[#0A3825]'
                            ...
                            >
                              <img
                                ...
                                className="w-10 h-10 rounded-full object-cover border border-[#D4AF37]/40 group-hover:scale-105 transition-transform shadow-xs shrink-0"
                              />
                              <div className="min-w-0">
                                <div className="text-xs font-semibold text-[#0A3825] group-hover:text-[#B45309] transition-colors truncate flex items-center gap-1">
                                  <span>{cat.name}</span>
                                  {isWaterBottle && (
                                    <span className="text-[9px] bg-[#D4AF37] text-[#0A3825] px-1 rounded-sm font-extrabold">NEW</span>
                                  )}
                                </div>
                                <div className="text-[10.5px] text-slate-500">{count} items</div>
                              </div>
                            </a>
                          );
                        })}
          ...
          <div className="flex items-center gap-1.5 sm:gap-3 shrink-0">
            {/* Search Trigger Button with ⌘K badge - Visible on desktop, accessed via bottom nav on mobile */}
            <button
              onClick={() => setIsSearchOpen(true)}
              className="hidden lg:flex items-center gap-2 px-3 py-1.5 rounded-full bg-slate-100/90 hover:bg-slate-200/80 text-slate-600 hover:text-[#0A3825] border border-slate-200/80 transition-all cursor-pointer group shadow-2xs"
              aria-label="Search"
            ...
            {currentUser ? (
              <div className="relative" ref={userMenuRef}>
                <button
                  ...
                  className={`flex items-center gap-1.5 sm:gap-2 p-1 sm:p-1.5 sm:pl-2.5 sm:pr-3 rounded-full border transition-all cursor-pointer group shadow-2xs shrink-0 ${
                    currentPage === 'profile'
                ...
                >
                  <div className="w-6 h-6 rounded-full bg-[#0A3825] text-amber-300 border border-[#D4AF37]/60 flex items-center justify-center font-serif font-bold text-xs shadow-inner shrink-0">
                  ...
                  <ChevronDown
                    className={`hidden sm:inline w-3.5 h-3.5 transition-transform duration-200 ${
              ...
              <button
                ...
                className="p-1.5 sm:p-2 text-slate-700 hover:text-[#0A3825] transition-colors relative flex items-center gap-1 cursor-pointer font-medium text-xs rounded-lg hover:bg-slate-100 shrink-0"
                title="Log In / Register"
              >
                <User className="w-5 h-5 text-[#0A3825]" />
                <span className="hidden md:inline font-semibold">Log In</span>
              </button>
            )}

            {/* Wishlist Icon - Hidden on mobile as it's permanently present in MobileBottomNav */}
            <button
              onClick={() => navigateToPage('wishlist')}
              className="hidden sm:flex text-slate-700 hover:text-[#D4AF37] p-2 transition-colors relative cursor-pointer shrink-0"
              aria-label="Wishlist"
            >
            ...
            <button
              ...
              className="bg-[#0A3825] hover:bg-[#062418] text-white border border-[#D4AF37]/40 p-1.5 sm:px-3 sm:py-2 rounded-xl transition-all flex items-center gap-1.5 sm:gap-2 shadow-xs hover:shadow-md shrink-0 cursor-pointer"
              aria-label="Cart"
            >
              <ShoppingBag className="w-4 h-4 sm:w-5 sm:h-5 text-amber-300" />
              <span className="hidden sm:inline text-xs font-semibold">Cart</span>
              {totalCartCount > 0 && (
                <span className="bg-[#D4AF37] text-[#0A3825] text-[10px] sm:text-xs font-bold w-4 h-4 sm:w-5 sm:h-5 rounded-full flex items-center justify-center">
                  {totalCartCount}
                </span>
              )}
            </button>
            ...
            >
              <div>
                ...
                <div className="mt-6 space-y-4">
                  ...
                  <div className="pl-3 space-y-1.5 my-2 max-h-60 overflow-y-auto pr-1">
                    <div className="flex items-center justify-between py-1 border-b border-slate-100">
                      <p className="text-[11px] uppercase font-bold text-[#B45309] tracking-wider">All Categories</p>
                    ...
                    {categories.map((cat, idx) => {
                      ...
                      return (
                        <button
                          ...
                          className={`w-full text-left py-1.5 px-2 rounded-lg text-xs transition-colors flex items-center justify-between cursor-pointer ${
                            isWaterBottle
                              ? 'bg-emerald-50 text-emerald-900 font-bold border border-emerald-200'
                              : 'text-slate-600 hover:text-[#0A3825] hover:bg-slate-50'
                          }`}
                        >
                          ...
                          <span className={`text-[10px] px-1.5 py-0.2 rounded-full font-medium ${isWaterBottle ? 'bg-emerald-200 text-emerald-950 font-bold' : 'bg-slate-100 text-slate-500'}`}>
                            {count}
                          </span>
                        </button>
                      );
