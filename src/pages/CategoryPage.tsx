...
export const CategoryPage: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-[#FAF9F6] pb-20 text-slate-800">
      {activeCategory && (
        <SEOHead
          title={`${activeCategory.name} - Buy Online | MS Home Trends`}
          description={
            activeCategory.description ||
            `Buy quality ${activeCategory.name} online at MS Home Trends. Explore our curated selection with Cash on Delivery and reliable doorstep delivery across Pakistan.`
          }
          keywords={`${activeCategory.name}, buy ${activeCategory.name} online, crockery, kitchen tools, drinkware, MS Home Trends Pakistan`}
          canonicalUrl={`/category/${categorySlug}`}
          ogImage={activeCategory.image}
          jsonLd={[
            buildCollectionSchema(activeCategory.name, activeCategory.description || `Quality ${activeCategory.name} from MS Home Trends.`, categoryProducts),
