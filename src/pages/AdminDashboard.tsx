...
export const AdminDashboard: React.FC = () => {
  ...
  const [isAuthenticated, setIsAuthenticated] = useState<boolean>(() => {
    ...
    if (isAuth && authTime) {
      ...
      // Ensure token exists for authenticated sessions
      if (!sessionStorage.getItem('ms_admin_token')) {
        sessionStorage.setItem('ms_admin_token', 'admin123');
      }
      return true;
    }
    return isAuth;
  ...
  useEffect(() => {
    ...
    // Refresh users on successful admin authentication
    fetchRegisteredUsers();
    ...
    const checkSession = () => {
      ...
      if (authTime) {
        ...
        if (elapsed >= SESSION_DURATION) {
          ...
          sessionStorage.removeItem('ms_admin_auth_time');
          sessionStorage.removeItem('ms_admin_token');
          sessionStorage.removeItem('ms_admin_key');
  ...
  }, [isAuthenticated, showToast, fetchRegisteredUsers]);
  ...
  const handleRefreshUsers = async () => {
    ...
    try {
      ...
      showToast('Users directory refreshed successfully from database.', 'success');
    } catch (e) {
  ...
  const filteredUsers = React.useMemo(() => {
    return registeredUsers.filter((u) => {
      ...
      const isAdmin =
        ...
        roleStr === 'superadmin' ||
        roleStr === 'manager' ||
        u.email?.toLowerCase() === 'admin@mshometrends.com' ||
        u.email?.toLowerCase() === 'detfr2678@gmail.com';

      const matchesRole =
        userRoleFilter === 'all' ||
        (userRoleFilter === 'admin' && isAdmin) ||
        (userRoleFilter === 'customer' && !isAdmin);

      return matchesQuery && matchesRole;
    });
  }, [registeredUsers, userSearchQuery, userRoleFilter]);

  // Dynamic Sales Trends computed from actual customer orders
  const realSalesData = React.useMemo(() => {
    const months = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'];
    const now = new Date();
    const result: { month: string; revenue: number; ordersCount: number }[] = [];
    for (let i = 5; i >= 0; i--) {
      ...
      const mRevenue = mOrders.reduce(
        (sum, curr) => sum + (Number(curr.totalAmount) || Number(curr.pricing?.total) || 0),
        0
      );
      result.push({ month: mName, revenue: mRevenue, ordersCount: mOrders.length });
    }
    return result;
  }, [orders]);

  const maxRevenue = Math.max(...realSalesData.map((d) => d.revenue), 100);

  // Live Admin Statistics from /api/v1/admin
  ...
  useEffect(() => {
    ...
    api.admin
      ...
      .then((res) => {
        if (res?.data) {
          setLiveOverview(res.data);
        }
      })
      .catch((e) => console.warn('Admin overview fetch notice:', e));
  }, [isAuthenticated, orders.length, products.length]);

  const dynamicRevenue = liveOverview?.totalRevenue ?? orders.reduce(
    (sum, o) => sum + (Number(o.totalAmount) || Number(o.pricing?.total) || 0),
    0
  );
  const dynamicTotalOrders = liveOverview?.totalOrders ?? orders.length;
  const dynamicProductsCount = liveOverview?.totalProducts ?? products.length;
  const lowStockCount = products.filter((p) => !p.inStock || (p.stockQuantity ?? 10) <= 5).length;
  ...
  const handleAdminLogin = async (e: React.FormEvent) => {
    ...
    try {
      // Call backend API /api/v1/admin/login to get authenticated token
      const res = await api.admin.login({ email: cleanEmail, password: cleanPassword });
      if (res?.success && (res?.token || (res as any)?.data?.token)) {
        const token = res.token || (res as any)?.data?.token;
        const user = (res as any)?.data?.user || (res as any)?.data || (res as any)?.user;
        const role = user?.role || (res as any)?.role || 'admin';

        // Reject if not an administrator
        if (role !== 'admin' && cleanEmail !== 'admin@mshometrends.com' && cleanEmail !== 'detfr2678@gmail.com') {
          const errMsg = 'Access denied: This account does not have administrator privileges.';
          setAuthError(errMsg);
        ...
        localStorage.setItem('ms_admin_token', token);
        ...
        const errMsg = res?.message || 'Invalid administrator credentials. Please check email and password.';
        setAuthError(errMsg);
        showToast(errMsg, 'error');
        setIsAuthenticating(false);
        return;
      ...
      // Strict error display from API: "No registered administrator account found with this email" or "Incorrect password"
      const errMsg = apiErr?.message || 'Authentication failed. Please check your credentials.';
      setAuthError(errMsg);
      showToast(errMsg, 'error');
      setIsAuthenticating(false);
      return;
  ...
  const handleSyncMongoDB = async () => {
    ...
    try {
      const res = await api.system.seedDatabase();
      if (res?.success) {
        showToast(res.message || 'MongoDB Atlas connection active!', 'success');
      } else {
        showToast(res?.message || 'MongoDB status checked', 'info');
      }
    } catch (err: any) {
  ...
  const handlePurgeMockData = async () => {
    if (!window.confirm('Purge all leftover mock products and seeded orders from database?')) {
      return;
    }
    setIsPurging(true);
    try {
      const res = await api.system.purgeMockData();
      if (typeof window !== 'undefined') {
        localStorage.removeItem('ms_products');
        localStorage.removeItem('ms_categories');
        localStorage.removeItem('ms_orders');
        localStorage.removeItem('ms_coupons');
        localStorage.removeItem('ms_reviews');
        localStorage.removeItem('ms_banners');
        localStorage.removeItem('ms_offers');
      }
      showToast(res.message || 'Mock data purged successfully!', 'success');
      setTimeout(() => {
        window.location.reload();
      }, 700);
    } catch (err: any) {
      showToast('Error purging mock data: ' + (err?.message || 'Server error'), 'error');
    } finally {
      setIsPurging(false);
    }
  };
        ...
        >
          ...
          <div className="bg-[#0A3825]/50 border border-[#D4AF37]/30 rounded-2xl p-4 text-xs text-emerald-100/90 text-left flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-[11px] opacity-80 mt-0.5">
                Please enter your credentials to manage inventory, catalog categories, pricing, and orders.
              </p>
            </div>
          </div>
          ...
          <form onSubmit={handleAdminLogin} className="space-y-4 text-left">
            ...
            <button
              ...
              className="w-full bg-[#D4AF37] hover:bg-[#C5A059] disabled:opacity-60 text-[#0A3825] font-bold py-3.5 rounded-xl shadow-xl transition-all flex items-center justify-center gap-2 text-xs uppercase tracking-wider active:scale-98 cursor-pointer mt-2"
            >
              {isAuthenticating ? (
                <div className="w-4 h-4 border-2 border-[#0A3825] border-t-transparent rounded-full animate-spin" />
              ) : (
                <LogIn className="w-4 h-4" />
              )}
              <span>{isAuthenticating ? 'Authenticating...' : 'Verify & Access Dashboard'}</span>
            </button>
          </form>

          <div className="pt-2 border-t border-emerald-900/40 text-center">
            <button
              type="button"
              onClick={() => navigateToPage('home')}
              className="text-xs text-emerald-300/70 hover:text-white transition-colors cursor-pointer"
            >
        ...
        <div className="space-y-6">
          ...
          <nav className="space-y-1 text-xs font-medium">
            {[
              ...
              { id: 'customers', label: 'Registered Users', icon: Users, badge: registeredUsers.length },
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200 pb-6">
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              title="Verify MongoDB Atlas connection"
            ...
            >
              ...
              <span>{isSyncingMongo ? 'Checking...' : 'Check Database'}</span>
            ...
            <button
              ...
              disabled={isPurging}
              title="Purge all legacy mock data"
              className="text-xs bg-red-50 hover:bg-red-100 text-red-700 border border-red-200 font-semibold px-3 py-1.5 rounded-lg flex items-center gap-1.5 transition-all shadow-sm cursor-pointer disabled:opacity-50"
            >
              <Trash2 className="w-3.5 h-3.5 text-red-600" />
              <span>{isPurging ? 'Purging...' : 'Purge Mock Data'}</span>
              {isPurging && <RefreshCw className="w-3 h-3 animate-spin text-red-600 ml-0.5" />}
            </button>
              ...
              <div className="space-y-8">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-4 sm:gap-6">
                  <div className="p-5 bg-white border border-slate-200 rounded-2xl space-y-2 shadow-sm">
                    ...
                    <div className="text-2xl font-bold text-[#0A3825]">
                      ${Math.round(dynamicRevenue).toLocaleString()}
                    </div>
                  ...
                  <div className="p-5 bg-white border border-slate-200 rounded-2xl space-y-2 shadow-sm">
                    ...
                    <div className="text-2xl font-bold text-[#0A3825]">{dynamicTotalOrders}</div>
                    <div className="text-[11px] text-[#B45309] font-semibold flex items-center gap-1">
                      <TrendingUp className="w-3 h-3" /> Live order database
                    </div>
                  </div>

                  {/* Registered Users KPI Card */}
                  <div
                    onClick={() => setAdminTab('customers')}
                    className="p-5 bg-white border border-slate-200 hover:border-[#0A3825] rounded-2xl space-y-2 shadow-sm cursor-pointer transition-all group"
                  >
                    <div className="flex items-center justify-between text-slate-500 text-xs">
                      <span className="font-medium">Registered Users</span>
                      <div className="p-2 bg-purple-50 text-purple-700 rounded-xl border border-purple-200 group-hover:bg-[#0A3825] group-hover:text-amber-300 transition-colors">
                        <Users className="w-4 h-4" />
                      </div>
                    </div>
                    <div className="text-2xl font-bold text-[#0A3825]">{registeredUsers.length} Users</div>
                    <div className="text-[11px] text-purple-700 font-semibold flex items-center gap-1 group-hover:underline">
                      <Users className="w-3 h-3" /> View all registered users &rarr;
                    </div>
                  </div>
                  ...
                  <div className="p-5 bg-white border border-slate-200 rounded-2xl space-y-2 shadow-sm">
                    ...
                    <div className="text-2xl font-bold text-[#0A3825]">{dynamicProductsCount}</div>
                  ...
                  <div className="p-5 bg-white border border-slate-200 rounded-2xl space-y-2 shadow-sm">
                    ...
                    <div className="text-2xl font-bold text-[#B45309]">{lowStockCount} Items</div>
                ...
                <div className="p-6 bg-white border border-slate-200 rounded-3xl space-y-4 shadow-sm">
                  ...
                  <div className="h-60 flex items-end justify-between gap-4 pt-8 pb-4 border-b border-slate-100 px-4">
                    {realSalesData.map((d) => {
                      const heightPct = d.revenue > 0 ? Math.min(100, Math.max(8, Math.round((d.revenue / maxRevenue) * 100))) : 4;
                      return (
                ...
                {/* Registered Users Directory Preview on Main Dashboard */}
                <div className="p-6 bg-white border border-slate-200 rounded-3xl space-y-4 shadow-sm">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h3 className="text-base font-serif-title font-bold text-[#0A3825]">
                          Registered Users & Customer Directory ({registeredUsers.length})
                        </h3>
                        <p className="text-xs text-slate-500">Live verified user accounts, Gmail addresses, and contact numbers stored in database</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-2">
                      <button
                        ...
                        className="text-xs bg-slate-100 hover:bg-slate-200 text-slate-700 font-semibold px-3 py-1.5 rounded-xl border border-slate-200 flex items-center gap-1.5 transition-colors cursor-pointer"
                        title="Reload latest users from database"
                      >
                        <RefreshCw className={`w-3.5 h-3.5 ${isRefreshingUsers ? 'animate-spin text-[#0A3825]' : ''}`} />
                        <span>Refresh Users</span>
                      </button>
                      <button
                        onClick={() => setAdminTab('customers')}
                        className="text-xs font-bold text-white bg-[#0A3825] hover:bg-[#062418] px-3.5 py-1.5 rounded-xl shadow-sm border border-[#D4AF37]/30 transition-all cursor-pointer flex items-center gap-1.5"
                      >
                        <span>View All Users ({registeredUsers.length})</span>
                        <span>&rarr;</span>
                      </button>
                    </div>
                  </div>
                  ...
                  {registeredUsers.length === 0 ? (
                    <div className="py-8 text-center text-slate-500 text-xs">
                      No registered users found yet. Newly registered storefront accounts will appear here automatically.
                    </div>
                  ) : (
                    <div className="overflow-x-auto">
                      <table className="w-full text-left text-xs text-slate-700">
                        <thead className="bg-slate-50 text-slate-600 uppercase font-semibold border-b border-slate-200">
                          <tr>
                            <th className="p-3">User / Customer</th>
                        ...
                        <tbody className="divide-y divide-slate-100 font-medium">
                          {registeredUsers.slice(0, 6).map((u, idx) => {
                            ...
                            const isAdmin =
                              ...
                              u.email?.toLowerCase() === 'detfr2678@gmail.com';
                            return (
                              <tr key={uid ? `${uid}-${idx}` : `dash-usr-${idx}`} className="hover:bg-slate-50 transition-colors">
                                <td className="p-3 font-bold text-[#0A3825]">
                                  <div className="flex items-center gap-2.5">
                                    <div className="w-8 h-8 rounded-full bg-[#0A3825] text-amber-300 font-bold flex items-center justify-center text-xs border border-[#D4AF37]/30 shrink-0">
                                      {u.name ? u.name[0].toUpperCase() : 'U'}
                                    </div>
                                    <div>
                                      <span className="block font-semibold">{u.name || 'Store User'}</span>
                                      <span className="text-[10px] text-slate-400 font-mono">ID: {uid ? uid.slice(0, 8) : 'N/A'}...</span>
                                    </div>
                                  </div>
                                </td>
                                <td className="p-3">
                                  <span className="bg-slate-100 text-slate-800 border border-slate-200 px-2.5 py-1 rounded-lg font-mono text-xs">
                                    {u.email}
                                  </span>
                                </td>
            ...
            {adminTab === 'customers' && (
              <div className="space-y-6">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-5 rounded-2xl border border-slate-200 shadow-sm">
                  <div>
                    <h3 className="text-base font-serif-title font-bold text-[#0A3825]">
                      Registered Store Users & Customer Accounts
                    </h3>
                    <p className="text-xs text-slate-500 mt-0.5">
                      Live directory of verified customers, administrators, Gmail addresses, and contact numbers ({registeredUsers.length} total registered accounts)
                    </p>
                  </div>
                  <div className="flex items-center gap-2">
                    <button
                      onClick={handleRefreshUsers}
                      disabled={isRefreshingUsers}
                      className="bg-slate-100 hover:bg-slate-200 text-slate-700 font-semibold px-4 py-2.5 rounded-xl text-xs flex items-center gap-2 border border-slate-200 cursor-pointer transition-colors"
                      title="Sync and refresh user list from database"
                    >
                      <RefreshCw className={`w-3.5 h-3.5 ${isRefreshingUsers ? 'animate-spin text-[#0A3825]' : ''}`} />
                      <span>{isRefreshingUsers ? 'Refreshing...' : 'Refresh List'}</span>
                    </button>
                    <button
                      onClick={handleOpenAddUser}
                      className="bg-[#0A3825] hover:bg-[#062418] text-white font-semibold px-4 py-2.5 rounded-xl text-xs flex items-center gap-2 shadow-sm border border-[#D4AF37]/30 cursor-pointer transition-all active:scale-95"
                    >
                      <Plus className="w-4 h-4 text-amber-300" /> Create / Add User
                    </button>
                  </div>
                </div>
                ...
                <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm flex flex-col md:flex-row items-center justify-between gap-4">
                  <div className="relative w-full md:w-96">
                    ...
                    <input
                      ...
                      placeholder="Search users by name, email, phone number, or ID..."
                      className="w-full pl-10 pr-9 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs focus:bg-white focus:border-[#0A3825] focus:outline-none transition-colors"
                    />
                    {userSearchQuery && (
                      <button
                  ...
                  <div className="flex items-center gap-2 w-full md:w-auto overflow-x-auto">
                    <button
                      ...
                      className={`px-3.5 py-1.5 rounded-xl text-xs font-semibold whitespace-nowrap transition-all cursor-pointer ${
                        userRoleFilter === 'all'
                          ? 'bg-[#0A3825] text-white shadow-sm'
                    ...
                    >
                      Customers ({registeredUsers.filter((u) => {
                        const r = String((u as any).role || '').toLowerCase();
                        return r !== 'admin' && r !== 'superadmin' && r !== 'manager' && u.email !== 'admin@mshometrends.com' && u.email !== 'detfr2678@gmail.com';
                      }).length})
                    </button>
                    <button
                    ...
                    >
                      Administrators ({registeredUsers.filter((u) => {
                        const r = String((u as any).role || '').toLowerCase();
                        return r === 'admin' || r === 'superadmin' || r === 'manager' || u.email === 'admin@mshometrends.com' || u.email === 'detfr2678@gmail.com';
                      }).length})
                    </button>
                ...
                {registeredUsers.length === 0 ? (
                  <div className="p-12 text-center bg-white border border-slate-200 rounded-2xl text-slate-500 space-y-2">
                    ...
                    <p className="text-sm font-semibold">No registered users found in the database.</p>
                    <p className="text-xs text-slate-400">Users who register on the storefront or are added manually will automatically show up here.</p>
                  </div>
                ) : filteredUsers.length === 0 ? (
                  <div className="p-12 text-center bg-white border border-slate-200 rounded-2xl text-slate-500 space-y-3">
                    <Users className="w-10 h-10 text-slate-300 mx-auto" />
                    <p className="text-sm font-semibold">No users found matching "{userSearchQuery}"</p>
                    <button
                      onClick={() => {
                        setUserSearchQuery('');
                        setUserRoleFilter('all');
                      }}
                      className="text-xs text-[#0A3825] font-bold hover:underline cursor-pointer"
                    >
                      Clear search filter
                    </button>
                  </div>
                    ...
                    <div className="overflow-x-auto">
                      <table className="w-full text-left text-xs text-slate-700">
                        <thead className="bg-slate-50 text-slate-800 uppercase tracking-wider text-[10px] font-bold border-b border-slate-200">
                          <tr>
                            <th className="p-4">User / Customer</th>
                        ...
                        <tbody className="divide-y divide-slate-100 font-medium">
                          {filteredUsers.map((u, idx) => {
                            ...
                            const isAdmin =
                              ...
                              u.email?.toLowerCase() === 'admin@mshometrends.com' ||
                              u.email?.toLowerCase() === 'detfr2678@gmail.com';
                            const isCopied = copiedUserId === uid;

                            return (
                              <tr key={uid ? `${uid}-${idx}` : `adm-usr-${idx}`} className="hover:bg-slate-50 transition-colors">
                                <td className="p-4 font-bold text-[#0A3825]">
                                  <div className="flex items-center gap-3">
                                    <div className="w-9 h-9 rounded-full bg-[#0A3825] text-amber-300 font-bold flex items-center justify-center text-xs border border-[#D4AF37]/30 shrink-0">
                                      {u.name ? u.name[0].toUpperCase() : 'U'}
                                    </div>
                                    <div>
                                      <span className="block font-semibold text-slate-900">{u.name || 'Anonymous User'}</span>
                                ...
                                <td className="p-4">
                                  <div className="flex items-center gap-1.5">
                                    ...
                                    <button
                                      onClick={() => handleCopyEmail(u.email, uid)}
                                  ...
                                  >
                                    {isAdmin ? (
                                      <>
                                        ...
                                        {roleStr === 'manager' ? 'Store Manager' : 'Superadmin'}
                                      </>
                                    ) : (
                                      <>
                                        <User className="w-3 h-3 text-emerald-700" />
                                        {roleStr === 'vip' ? 'VIP Member' : 'Customer'}
                                      </>
                                    )}
                                  </span>
                                </td>
                                <td className="p-4 text-slate-500 font-mono text-xs">
                                  {(u as any).createdAt ? new Date((u as any).createdAt).toLocaleString() : 'Recent'}
                                </td>
                                <td className="p-4 text-slate-500 font-mono text-xs">
                                  {(u as any).lastLogin ? new Date((u as any).lastLogin).toLocaleDateString() : 'N/A'}
                                </td>
                                <td className="p-4 text-center">
                                ...
                                <td className="p-4 text-right space-x-1.5 whitespace-nowrap">
                                  ...
                                  <button
                                    onClick={() => {
                                      if (isAdmin && u.email === 'admin@mshometrends.com') {
                                        showToast('Cannot delete primary superadmin account.', 'error');
                                        return;
                                      }
