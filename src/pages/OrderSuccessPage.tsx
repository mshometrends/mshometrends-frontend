...
export const OrderSuccessPage: React.FC = () => {
  ...
  const fetchOrderDetails = async () => {
    ...
    try {
      ...
      if (orderData && (orderData.orderId || orderData.id || orderData._id)) {
        setOrder(orderData);
      } else {
        setError((res as any)?.message || 'Order not found.');
      }
    } catch (err: any) {
  ...
  const handleUploadScreenshot = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch(ENDPOINTS.ORDERS.PAYMENT_SCREENSHOT(orderId), {
        method: 'POST',
        body: formData,
      });
