...
export const TrackOrderPage: React.FC = () => {
  ...
  const handleSearch = async (queryToSearch: string) => {
    ...
    try {
      setLoading(true);
      setError(null);
      const res = await api.orders.track(queryToSearch.trim());
      const orderList = Array.isArray(res) ? res : (res as any)?.data;
  ...
  const handleUploadScreenshot = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch(ENDPOINTS.ORDERS.PAYMENT_SCREENSHOT(activeUploadOrder.orderId), {
        method: 'POST',
        body: formData,
      });
