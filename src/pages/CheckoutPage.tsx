...
export const CheckoutPage: React.FC = () => {
  ...
  const handleSubmitOrder = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const createdOrder = (res as any)?.data || (res as any)?.order || res;

      if (createdOrder && createdOrder.orderId) {
        clearCart();
        navigate(`/order-success/${createdOrder.orderId}`);
      } else {
        setErrorMessage((res as any)?.message || 'Failed to place order. Please try again.');
      }
    } catch (err: any) {
      setErrorMessage(err?.message || 'Network error while placing order. Please check your connection.');
    } finally {
      setSubmitting(false);
    }
  };
