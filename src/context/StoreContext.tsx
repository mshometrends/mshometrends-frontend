...
export const getRouteFromPath = (path: string): { page: PageView; categorySlug?: string; productId?: string } => {
  ...
  // Admin route exact match & aliases: msowner/admin/login, msowner/admin, etc.
  if (
    cleanPath === 'msowner/admin/login' ||
    cleanPath === 'msowner/admin' ||
    cleanPath === 'msowner/admin/panel' ||
    cleanPath === 'msowner/admin/dashboard' ||
...
export const StoreProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [registeredUsers, setRegisteredUsers] = useState<User[]>(() => {
    if (typeof window !== 'undefined') {
      const saved = localStorage.getItem('ms_registered_users');
      if (saved) {
        try {
  ...
  const fetchRegisteredUsers = async () => {
    try {
      ...
      if (usersList.length > 0) {
        ...
        if (typeof window !== 'undefined') {
          localStorage.setItem('ms_registered_users', JSON.stringify(usersList));
        }
      }
    } catch (err) {
